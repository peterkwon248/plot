...
#### C-1.5. DetailPanel 이전/다음 네비게이션 버그 수정
- **현재 버그**: `items` 배열에서 currentIndex를 계산할 때, 필터되지 않은 전체 items를 사용
- **문제**: "메모" 뷰에서 이전/다음을 누르면 "완료" 상태 아이템으로 이동할 수 있음
- **수정**: 현재 뷰에서 필터된 아이템 목록 기준으로 네비게이션
- 파일: `components/layout/DetailPanel.tsx:26` — `items` 대신 뷰 필터된 목록 사용
