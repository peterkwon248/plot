    ...
    >
      ...
      >
        ...
        <div className="mb-6 pb-4 border-b border-white/[0.08]">
          ...
          <p className="text-sm text-[#9e96b4] mt-1">
        ...
        {!canUseAi ? (
          <div className="py-8 text-center">
            ...
            <button
              onClick={onClose}
              className="mt-6 px-4 py-2 bg-white/[0.04] hover:bg-white/[0.08] text-[#aea6c0] hover:text-[#e8e8f0] rounded-xl text-sm transition-all duration-200"
            >
              닫기
            </button>
          </div>
          ...
          <>
            ...
            <div className="mb-4 p-3 bg-white/[0.03] border border-white/[0.08] rounded-lg">
              <p className="text-xs text-[#aea6c0] mb-2">현재 스텝 ({steps.length}개)</p>
              <div className="flex flex-col gap-1.5">
                {steps.slice(0, 3).map((step, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="text-xs text-[#9e96b4] mt-0.5">{i + 1}.</span>
                    <span className="text-xs text-[#e8e8f0] line-clamp-1">{step.action}</span>
                  </div>
                ))}
                {steps.length > 3 && (
                  <span className="text-xs text-[#9e96b4]">외 {steps.length - 3}개...</span>
                )}
              </div>
            </div>
            ...
            {!newSteps && (
              <>
                <div className="mb-4">
                  <label className="text-xs text-[#aea6c0] block mb-2">
                    어떻게 수정할까요?
                  </label>
                  <textarea
                    value={userMessage}
                    ...
                    className="w-full px-3 py-2.5 bg-[#1a1a1f] border border-white/[0.08] rounded-lg text-sm text-[#e8e8f0] placeholder:text-[#9e96b4] resize-none focus:border-[#a78bfa]/30 focus:outline-none transition-colors"
                    rows={4}
                    disabled={isLoading}
                  />
                </div>
                ...
                <div className="flex gap-3">
                  <button
                    onClick={onClose}
                    disabled={isLoading}
                    className="flex-1 px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] text-[#aea6c0] hover:text-[#e8e8f0] rounded-xl text-sm transition-all duration-200 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
                    취소
                  </button>
                  <button
            ...
            {newSteps && (
              <>
                <div className="mb-4">
                  <p className="text-xs text-[#aea6c0] mb-3">수정된 스텝 미리보기</p>
                    ...
                    {newSteps.map((step, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 ml-7">
                          <span className="text-xs text-[#9e96b4]">{step.minutes}분</span>
                          <span className="text-xs text-[#9e96b4]">·</span>
                          <span className="text-xs text-[#aea6c0]">{step.resultType}</span>
                        </div>
                      </div>
                    ))}
                  </div>
